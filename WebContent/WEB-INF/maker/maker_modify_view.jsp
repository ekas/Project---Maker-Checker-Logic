...
<%String recStatus = (String)request.getAttribute("recStatus1");
session.setAttribute("recStatus2", recStatus);%>
<%=recStatus%>
...
<body>
  ...
  <div class="main">
    <div class="container-custom">
      ...
      <h3><%= request.getAttribute("ret")%></h3>
      

      <form action="http://localhost:8081/WebApp2CustInventoryBrd/formServlet" method="get">
		<!--  
      <div class="form-group">
          <label>Operation*</label>
          <select id="operation" class="custom-combobox" name="operation">
            <option value="-1">Select one Operation</option>
      ...
      <!--2-->
        <div class="form-group">
          ...
          <input id="custCode" type="text" class="form-control"  placeholder="Enter Customer Code" name="custCode" value="${cus.getCustCode()}" readonly="readonly">
        </div>
      <!--3-->
        <div class="form-group">
          <label>Customer Name*</label>
          <input id="custName" type="text" class="form-control" placeholder="Enter Customer Name" name="custName" value="${cus.getCustName()}">
        </div>
      <!--4-->
        <div class="form-group">
          <label>Customer Address Line 1*</label>
          <textarea id="custAdd1" rows="2" class="form-control" placeholder="Enter Customer Address Line 1" name="custAdd1" >${cus.getCustAdd1()}</textarea>
        </div>
      <!--5-->
        <div class="form-group">
          <label>Customer Address Line 2</label>
          <textarea id="custAdd2" rows="2" class="form-control" placeholder="Enter Customer Address Line 2" name="custAdd2">${cus.getCustAdd2()}</textarea>
        </div>
      <!--6-->
        <div class="form-group">
          <label>Customer Pin Code*</label>
          <input id="custPin" type="text" class="form-control" placeholder="Enter Customer Pin Code" name="custPin" value="${cus.getCustPin()}">
        </div>
      <!--7-->
        <div class="form-group">
          <label>Email address*</label>
          <input id="custEmail" type="email" class="form-control" placeholder="Enter Email Address" name="custEmail" value="${cus.getCustEmail()}">
        </div>
      <!--8-->
        <div class="form-group">
          <label>Customer Contact Number</label>
          <input id="custContact" type="text" class="form-control" placeholder="Enter Customer Contact Number" name="custContact" value="${cus.getCustContact()}">
        </div>
      <!--9-->
        <div class="form-group">
          <label>Primary Contact Person*</label>
          <input id="custPriContact" type="text" class="form-control" placeholder="Enter Primary Contact Person" name="custPriContact" value="${cus.getCustPriContact()}">
        </div>
      <!--10-->
        <div class="form-group">
          <label>Record Status*</label>
         <!--<input id="recStatus" type="text" class="form-control" placeholder="Enter Record Status" name="recStatus" value="${cus.getRecStatus()}">--> 
          <select id="recStatus" class="combobox custom-combobox" name="recStatus">
            <option value="-1">${cus.getRecStatus()}</option>
      ...
      <!--11-->
        <div class="form-group">
          <label>Active / Inactive Flag*</label>
          <select id="custAIFlag" class="combobox custom-combobox" name="custAIFlag">
            <option value="-1">${cus.getCustAIFlag()}</option>
        ...
        <div class="form-group">
          ...
          <input id="createBy" type="text" class="form-control" placeholder="Enter Creater ID " name="createBy" value="${cus.getCreateBy()}"readonly="readonly">
        </div>
         
        
        <div class="form-group">
          <label>Create Date*</label>
          <input id="createDate" type="text" class="form-control" placeholder="Enter Creating Date format - DD/MON/YYYY" name="createDate" value="${cus.getCreateDate()}" readonly="readonly">
        </div>
        
        <div class="form-group">
          <label>Modified Date</label>
          <input id="modDate" type="text" class="form-control" placeholder="Enter Modifying Date format - DD/MON/YYYY" name="modDate" value="${cus.getModDate()}" readonly="readonly">
        </div>
        <div class="form-group">
          <label>Modified By</label>
          <input id="modBy" type="text" class="form-control" placeholder="Enter Modifier ID" name="modBy" value="${cus.getModBy()}" disabled>
        </div>
        <div class="form-group">
          <label>Authorized Date</label>
          <input id="authDate" type="text" class="form-control" placeholder="Enter Authorizing Date format - DD/MON/YYYY" name="authDate" value="${cus.getAuthDate()}" readonly="readonly">
        </div>
        <div class="form-group">
        ...
        <button type="submit" id="btn-update" class="btn-update btn btn-default" name="btn" value="Back to Select M">Modify</button>
  ...
  //Validation on Maker-Checker
  $("#btn-update").click(function(){
    //var operation = $('#operation').val();
    var custCode = $('#custCode').val();
    var custName = $('#custName').val();
    var custAdd1 = $('#custAdd1').val();
    var custAdd2 = $('#custAdd2').val();
    var custPin = $('#custPin').val();
    var custEmail = $('#custEmail').val();
    var custContact = $('#custContact').val();
    var custPriContact = $('#custPriContact').val();
    var recStatus = $('#recStatus').val();
    var custAIFlag = $('#custAIFlag').val();
    var createDate = $('#createDate').val();
    var createBy = $('#createBy').val();
    var modDate = $('#modDate').val();
    var modBy = $('#modBy').val();
    var authDate = $('#authDate').val();
    var authBy = $('#authBy').val();


    var regexCode = /^[a-zA-Z0-9]+$/;
    var regexName = /*/^[a-zA-Z0-9\\s]+$/; *//^[a-zA-Z ]+$/;
    var regexEmail = /^[\w\-\.\+]+\@[a-zA-Z0-9\.\-]+\.[a-zA-z0-9]{2,4}$/;
    var regexAdd = /^[0-9a-zA-Z-, ]+$/;
    var regexPin = /^[0-9]+$/;
    var regexPhone = /^[0-9]+$/;
    //var regexDate = /^[0-9]{2}/[a-zA-Z]{3}/[0-9]{4}+$/;
    
    console.log(custCode);
    if (custCode.length === 0 || custName.length === 0 || custAdd1.length === 0 || custPin.length === 0 || custEmail.length === 0 || custPriContact.length === 0 || recStatus === "-1" || custAIFlag == "-1") {
      alert("* Marked Fields are mandatory");
      return false;
    }

    if(!regexCode.test(custCode)){
      console.log(custCode);
      alert("Customer Code Format not Valid - Must be only characters and numbers");
      $("#custCode").focus();
      return false;
    }

    if(!regexName.test(custName)){
      alert("Customer Name Format not Valid - Must be only characters");
      $("#custName").focus();
      return false;
    }

    if(!regexAdd.test(custAdd1)){
      alert("Customer Address Format not Valid - Must be only characters and Special Characters like - ,  allowed");
      $("#custAdd1").focus();
      return false;
    }

    if(!regexPin.test(custPin)){
      alert("Customer Pin Code Format not Valid - Must be only Numbers");
      $("#custPin").focus();
      return false;
    }

    if(!regexEmail.test(custEmail)){
      alert("Customer Email Format not Valid");
      $("#custEmail").focus();
      return false;
    }

    if(!regexPhone.test(custContact)){
      alert("Customer Contact Number not Valid - Must be only Numbers");
      $("#custContact").focus();
      return false;
    }

    if(!regexName.test(custPriContact)){
      alert("Customer Primary Contact not Valid - Must be only Numbers");
      $("#custPriContact").focus();
      return false;
    }
    
    if(!regexCode.test(createBy)){
      alert("Create ID not Valid - Must be only Characters and Numbers");
      $("#createBy").focus();
      return false;
    }
/*
    if(!regexDate.test(createDate)){
      alert("Create Date not Valid - Format - DD/MON/YYYY");
      $("#createDate").focus();
    return false;
    }


    if(!regexDate.test(modDate)){
      alert("Create Date not Valid - Format - DD/MON/YYYY");
      $("#modDate").focus();
    return false;
    }

    if(!regexCode.test(modBy)){
      alert("Create ID not Valid - Must be only Characters and Numbers");
      $("#modBy").focus();
    return false;
    }

    if(!regexDate.test(authDate)){
      alert("Create Date not Valid - Format - DD/MON/YYYY");
      $("#authDate").focus();
    return false;
    }

    if(!regexCode.test(authBy)){
      alert("Create ID not Valid - Must be only Characters and Numbers");
      $("#authBy").focus();
    return false;
    }
